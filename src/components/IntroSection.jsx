...
/** Google Form RSVP — https://forms.gle/wWs6wnWRHd3BghB96 */
const RSVP_FORM_EMBED_SRC =
  'https://docs.google.com/forms/d/e/1FAIpQLSf-v2hvEPrRf6XSN5Y96-LBpIWqvaBZ046CIwDZzkWPuWdfCQ/viewform?embedded=true'
      ...
      <div className="relative z-20 mx-auto max-w-sm px-5 sm:px-7">
        ...
        <div className="intro-content-soft-panel px-7 py-10 text-center sm:px-10 sm:py-12 md:px-12 md:py-14">
          <div ref={calendarRef} className="w-4/5 mx-auto origin-top scale-125 sm:scale-[1.35]">
            ...
            <div className="mt-3 grid grid-cols-7 gap-y-1 text-center text-xs font-poppins uppercase" style={{ color: '#8B5560' }}>
              {weekLabels.map((label, index) => (
                <span key={`${label}-${index}`}>{label}</span>
              ))}
            </div>
            <div className="mt-1 grid grid-cols-7 gap-y-1 text-center text-sm font-poppins" style={{ color: '#8B5560' }}>
        ...
        <div ref={buttonWrapRef} className="mt-6 flex w-full justify-center">
          ...
          >
            ...
            <FiMail className="h-5 w-5 shrink-0 text-[#B07D86]" aria-hidden="true" />
          </GraphicButton>
        </div>
      </div>
      {isRsvpModalOpen && (
        <div className="fixed inset-0 z-[120] flex items-end justify-center px-0">
          <div
            ...
            aria-hidden
          />
          <div className="modal-slide-up-panel relative w-screen h-screen bg-[#FBF3F0] p-4 sm:p-6 shadow-[0_18px_45px_rgba(0,0,0,0.28)]">
            <button
              type="button"
              onClick={() => setIsRsvpModalOpen(false)}
              className="absolute right-4 top-4 z-10 font-poppins text-xs uppercase tracking-[0.2em] text-[#8B5560]"
            >
              Close
            </button>
